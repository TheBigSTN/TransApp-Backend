...
public class CursaUtil {
    ...
    public InputStreamResource generatorPdfItext(Anexa anexa) {
        ...
        try {
            ...
            document.add(new Paragraph("                S.C. ____SGM TRANS____ S.R.L.", font));
            document.add(
                    new Paragraph("                Nr. Ord. Reg. Com. J_23_/_162_/_2008 CUI R O 23116668______", font));
            ...
            Font rowFont = font;
            rowFont.setSize(9);
            log.info(rowFont.toString());
            addRows(table, cursaList, rowFont);
            addLastLine(table, anexa);
            document.add(table);

            document.add(new Paragraph(" ", font));
            document.add(new Paragraph(" ", font)); // Add empty lines for spacing
            document.add(new Paragraph(
                    "                    SEMNATURA SI STAMPILA FURNIZORULUI:              Total cu TVA: ___"
                            + String.valueOf(anexa.getValoare() + anexa.getTva()) + "___ ",
                    font));
            document.add(new Paragraph("                    S.C. ______SGM TRANS_______ S.R.L.", font));
    ...
    private void addLastLine(PdfPTable table, Anexa anexa) {
        ...
        addCell(table, String.valueOf(anexa.getCurse().size() + 1), Element.ALIGN_CENTER, cellHeight, headerFont); // Empty
                                                                                                                   // cell
                                                                                                                   // for
                                                                                                                   // Km
